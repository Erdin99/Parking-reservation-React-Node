...
const createComment = async (parking_id, created_comment_by_id, created_comment_by_username, created_comment_by_email, comment, grade) => {
    await db.raw(`INSERT INTO parking_comments 
...
const readParkingComments = async (id) => {
    const comments = await db.raw(`SELECT id, created_comment_by_username, created_comment_by_email, comment, grade, created_at FROM parking_comments WHERE parking_id = :id`, {id})
